...
public class Socket_Assistor implements Runnable{
...
//    private int SERVER_HOST_PORT = 333;
    ...
    public Socket_Assistor(Handler handler,String Host_IP,int Host_Port) throws IOException {
                ...
                public void run() {
                    try {
                        ...
                        while ((data_len=skinput.read(server_data))!=-1){
                            Message msg=recv_handler.obtainMessage();//recv server msg and send to recv_hander
                            msg.what=2;//recv flag
                            msg.obj=new String(server_data,0,data_len);
                            Log.i("server",msg.obj.toString());
                            recv_handler.sendMessage(msg);//send the server msg
